...
על עצמכם. ותגלו כמה גבולות נועדנו לפרוץ.`;
  ...
  return (
    <div className={`game-wrapper ${viewState === 'game-play' ? 'gray-theme' : 'red-theme'}`}>
      ...
      {/* 2. Instructions Screen - מותאם למובייל בלבד */}
      {viewState === 'instructions' && (
        <div className="center-box">
            <div className="glass-panel fade-in instruction-mobile-panel">
                <h2 className="panel-title">הוראות המשחק</h2>
                <div className="typewriter-scroll-container">
                ...
                {isTypingDone && (
                  <div className="instruction-mobile-btn-wrapper">
                    <button className="action-btn-neon-large" onClick={() => setViewState('category-selection')}>המשיכו עוד קצת</button>
                  </div>
                )}
            </div>
        </div>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; font-family: 'Open Sans', sans-serif !important; }
        ...
        .footer-centered-line { text-align: center; margin-top: 30px; font-size: 1.3rem; font-weight: 700 !important; display: block; color: #fff !important; }
        
        /* התאמות ספציפיות להוראות בנייד */
        @media (max-width: 768px) {
          .game-wrapper { height: 100dvh; }
          .instruction-mobile-panel { max-height: 85dvh; display: flex; flex-direction: column; padding: 20px; }
          .typewriter-scroll-container { flex: 1; overflow-y: auto; margin-bottom: 15px; padding-left: 5px; }
          .instruction-mobile-btn-wrapper { flex-shrink: 0; width: 100%; }
        }
