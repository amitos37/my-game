...
const MobileGamePage = () => {
  ...
  return (
    <div className={`game-wrapper ${viewState === 'game-play' ? 'gray-theme' : 'red-theme'}`}>
      ...
      {/* 2. Instructions Screen - תיקון חיתוך ואחידות */}
      {viewState === 'instructions' && (
        <div className="state-view-container instructions-outer">
            <div className="instr-card-container fade-in">
                ...
                <div className="instr-scroll-content luxury-scroll">
                    <div className="instr-block-para tight-text">
                    ...
                    <div className="instr-block-para margin-v-15 centered"><p>תמצאו במשחק...</p></div>
                    <div className="instr-block-para tight-text">
                    ...
                    <div className="instr-block-para margin-v-15 tight-text">
      ...
      {/* 3. Category Selection - שלג גלובלי בלבד והסרת שלג מהשורה */}
            ...
            <div key={currentCard.text} className={`active-card-wrapper ${isExiting ? 'throw-out' : 'deal-in'}`}>
              <div className={`flip-inner ${isCardOpen ? 'flipped' : ''}`}>
                ...
                <div className="flip-back">
                  ...
                  <div className="card-text-overlay"><span className="card-main-text">{currentCard.text}</span></div>
                </div>
              </div>
            </div>
          </div>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; font-family: 'Open Sans', sans-serif !important; }
        ...
        .state-view-container { width: 100%; height: 100%; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 20px; transition: background 0.7s ease; }
        ...
        /* Instructions Screen */
        .instructions-outer { gap: 20px; }
        .instr-card-container { width: 95%; max-width: 550px; max-height: 75vh; background: rgba(15, 15, 15, 0.92); border: 1px solid rgba(255,255,255,0.25); border-radius: 30px; padding: 35px 45px; direction: rtl; display: flex; flex-direction: column; box-shadow: 0 15px 50px rgba(0,0,0,0.6); flex-shrink: 1; overflow: hidden; }
        .instr-scroll-content { color: #fff; text-align: center; overflow-y: auto; width: 100%; flex-grow: 1; padding-right: 5px; }
        .instr-main-header { color: #fff; text-align: center; font-size: 2.1rem; margin-bottom: 15px; border-bottom: 1px solid rgba(255,255,255,0.15); padding-bottom: 10px; flex-shrink: 0; }
        .instr-block-para.tight-text p { line-height: 1.4; margin: 0; font-size: 1.1rem; }
        .instr-final-footer { font-size: 1.6rem; font-weight: 800; color: #fff; text-align: center; flex-shrink: 0; padding-top: 15px; }
        .instr-button-wrapper { width: 100%; display: flex; justify-content: center; flex-shrink: 0; }
        .instr-action-btn { padding: 18px 60px; font-size: 1.4rem; border-radius: 35px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.4); color: #fff; cursor: pointer; }

        @media (max-width: 768px) { .instr-card-container { padding: 25px; max-height: 72vh; } .instr-action-btn { padding: 15px 40px; font-size: 1.2rem; } }
        ...
        @keyframes vibeAnim { 0% { transform: translate(2px, 1px) rotate(0deg); } 50% { transform: translate(-2px, -1px) rotate(1deg); } 100% { transform: translate(2px, -1px) rotate(-1deg); } }
        ...
        ${[...Array(25)].map((_, i) => `.snowflake-particle:nth-child(${i+1}) { left: ${Math.random()*100}%; animation-delay: ${Math.random()*5}s; font-size: ${14 + Math.random()*18}px; }`).join('\n')}

        .ice-row-frost { background: rgba(224, 242, 254, 0.5) !important; border: 2px solid #fff !important; box-shadow: inset 0 0 25px rgba(255,255,255,0.7), 0 0 20px #fff; position: relative; }
        ...
        .card-main-text { color: #fff; font-size: 1.5rem; font-weight: 800 !important; direction: rtl; display: block; width: 100%; }
